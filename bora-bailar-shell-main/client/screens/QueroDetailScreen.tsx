...
// EVENT_TEMPLATES removed - now using API data
...
export default function QueroDetailScreen() {
  ...
  // Get stories from API data
  const videoStories = discoverData?.stories || [];

  // Map events to the format expected by EventCard
      ...
      >
        ...
        >
          {videoStories.length > 0 ? (
            videoStories.map((story: any, index: number) => (
              <VideoStoryCard
                key={story.id}
                title={story.title}
            ...
            <View style={styles.emptyStoriesContainer}>
              <Text style={styles.emptyStoriesText}>Nenhum vídeo disponível</Text>
            </View>
          )}
        </ScrollView>
